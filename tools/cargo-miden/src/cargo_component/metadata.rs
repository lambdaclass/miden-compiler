...
use super::core::registry::{Dependency, RegistryPackage};
...
impl FromStr for Ownership {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            _ => Err(format!(
                "unrecognized ownership: `{s}`; expected `owning`, `borrowing`, or \
...
impl FromStr for Target {
    ...
    fn from_str(s: &str) -> Result<Self> {
        let (name, version) = match s.split_once('@') {
            Some((name, version)) => (
                name,
                version.parse().with_context(|| format!("invalid target version `{version}`"))?,
            ),
                ...
                match (entry.path, entry.package) {
                        ...
                        for (present, name) in [(!entry.dependencies.is_empty(), "dependencies")] {
                            if present {
                                return Err(de::Error::custom(format!(
                                    "cannot specify both `{name}` and `package` fields in a \
                                     target entry"
                                )));
                            }
                        }
                    ...
                    (path, None) => {
                        for (present, name) in [
                            ...
                            if present {
                                return Err(de::Error::custom(format!(
                                    "cannot specify both `{name}` and `path` fields in a target \
                                     entry"
                                )));
                            }
                        }
...
impl ComponentMetadata {
    ...
    pub fn from_package(package: &Package) -> Result<Self> {
        ...
        let manifest_dir =
            package.manifest_path.parent().map(|p| p.as_std_path()).with_context(|| {
                format!(
                    "manifest path `{path}` has no parent directory",
                    path = package.manifest_path
                )
            })?;
        let modified_at = super::last_modified_time(package.manifest_path.as_std_path())?;
