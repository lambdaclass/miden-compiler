...
use super::{
    core::registry::DecodedDependency, metadata::Ownership, registry::PackageDependencyResolution,
};
...
impl<'a> BindingsGenerator<'a> {
    ...
    pub async fn new(
        ...
        match Self::create_target_world(resolution, &mut import_name_map).await.with_context(
            || {
                format!(
    ...
    pub fn generate(self) -> Result<String> {
        ...
        let sources: Vec<_> = files
            ...
            .collect();
        assert!(sources.len() == 1, "expected exactly one source file to be generated");
    ...
    async fn target_package(
        ...
        let (resolve, pkg, source_files) =
            dependency.decode().await?.resolve().with_context(|| {
                format!("failed to resolve target package `{name}`", name = dependency.name())
            })?;
    ...
    async fn target_local_path(
        ...
        for (id, resolution) in &resolution.target_resolutions {
            ...
            if let Some(prev) = deps.insert(name.clone(), decoded) {
                bail!(
                    "duplicate definitions of package `{name}` found while decoding target \
                     dependency `{id}`",
        ...
        let root = if path.is_dir() {
            UnresolvedPackageGroup::parse_dir(path).with_context(|| {
                format!("failed to parse local target from directory `{}`", path.display())
            })?
        } else {
            UnresolvedPackageGroup::parse_file(path).with_context(|| {
                format!("failed to parse local target `{path}`", path = path.display())
            })?
        };

        let mut source_files: Vec<_> =
            root.source_map.source_files().map(Path::to_path_buf).collect();
        ...
        let package = merged.push_group(root).with_context(|| {
            format!("failed to merge local target `{path}`", path = path.display())
        })?;

        let world = merged.select_world(package, world).with_context(|| match world {
            Some(world) => {
                format!(
                ...
                DecodedDependency::Wit {
                    ...
                    for name in package.main.foreign_deps.keys() {
                        if !visiting.insert(name) {
                            bail!(
                                "foreign dependency `{name}` forms a dependency cycle while \
                                 parsing target dependency `{other}`",
    ...
    fn import_world(
        ...
        for (name, id) in interfaces {
                ...
                .is_some()
            {
                ...
                bail!(
                    "cannot import dependency `{id}` because it conflicts with an import in the \
                     target world"
                );
            }
        }
        ...
        if !functions.is_empty() {
            ...
            let name = format!("{ns}-{pkg}", ns = package.name.namespace, pkg = package.name.name);
            ...
            {
                bail!(
                    "cannot import dependency `{name}` because it conflicts with an import in the \
                     target world"
                );
            }
        }
