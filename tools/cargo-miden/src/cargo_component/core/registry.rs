...
use super::lock::{LockFileResolver, LockedPackageVersion};
    ...
    pub async fn decode(&self) -> Result<DecodedDependency> {
            ...
            return Ok(DecodedDependency::Wit {
                ...
                package: UnresolvedPackageGroup::parse(
                    ...
                    std::str::from_utf8(&bytes).with_context(|| {
                        format!("dependency `{name}` is not UTF-8 encoded", name = self.name())
                    })?,
                )?,
            });
        }
        ...
        Ok(DecodedDependency::Wasm {
            ...
            decoded: wit_component::decode(&bytes).with_context(|| {
                format!("failed to decode content of dependency `{name}`", name = self.name(),)
            })?,
        })
    }
}
    ...
    pub fn resolve(self) -> Result<(Resolve, PackageId, Vec<PathBuf>)> {
        match self {
            Self::Wit { package, .. } => {
                ...
                let source_files =
                    package.source_map.source_files().map(Path::to_path_buf).collect();
...
impl<'a> DependencyResolver<'a> {
        ...
        match dependency {
            Dependency::Package(package) => {
                ...
                let locked = match self.lock_file.as_ref().and_then(|resolver| {
                    resolver.resolve(registry_name, &package_name, &package.version).transpose()
                }) {
                ...
                registry.add_dependency(name, package_name, &package.version, locked).await?;
            }
            Dependency::Local(p) => {
    ...
    pub async fn resolve(mut self) -> Result<DependencyResolutionMap> {
        ...
        for resolution in
            self.registries.into_values().flat_map(|r| r.dependencies.into_iter()).map(|d| {
                DependencyResolution::Registry(
                    d.resolution.expect("dependency should have been resolved"),
                )
            })
        {
            let prev = self.resolutions.insert(resolution.name().clone(), resolution);
            assert!(prev.is_none());
        }
    ...
    async fn resolve(&mut self, registry: &'a str) -> Result<()> {
        for dependency in self.dependencies.iter_mut() {
            ...
            let (selected_version, digest) = if client.is_readonly() {
                dependency.locked.as_ref().map(|(ver, digest)| (ver, Some(digest))).ok_or_else(
                ...
                match &dependency.locked {
                    Some((version, digest)) => {
                        ...
                        find_latest_release(versions, &exact_req)
                            ...
                            .or_else(|| {
                                find_latest_release(versions, dependency.version)
                                    .map(|v| (&v.version, None))
                            })
                    }
                    None => find_latest_release(versions, dependency.version)
                        .map(|v| (&v.version, None)),
                }
                .with_context(|| {
                    format!(
                        "component registry package `{name}` has no release matching version \
                         requirement `{version}`",
                        name = dependency.package,
            ...
            // be worth replacing the mutable borrow with a RwLock down the line.
            let release = client.get_release(&dependency.package, selected_version).await?;
            if let Some(digest) = digest {
                if &release.content_digest != digest {
                    bail!(
                        "component registry package `{name}` (v`{version}`) has digest \
                         `{content}` but the lock file specifies digest `{digest}`",
                        name = dependency.package,
