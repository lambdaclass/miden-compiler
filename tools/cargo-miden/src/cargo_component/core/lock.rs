...
use super::registry::DEFAULT_REGISTRY_NAME;
    ...
    pub fn resolve(
        ...
        if let Some(pkg) = self
            ...
            .binary_search_by_key(
                &(package_ref.namespace().as_ref(), package_ref.name().as_ref(), registry),
                LockedPackage::key,
            )
            .ok()
            .map(|i| &self.0.packages[i])
        {
                ...
                .binary_search_by_key(&requirement.to_string().as_str(), LockedPackageVersion::key)
            {
                ...
                log::info!(
                    "dependency package `{package_ref}` from registry `{registry}` with \
                     requirement `{requirement}` was resolved by the lock file to version \
                     {version}",
        ...
        log::info!(
            "dependency package `{package_ref}` from registry `{registry}` with requirement \
             `{requirement}` was not in the lock file"
        );
        Ok(None)
    }
}
        ...
        match document.as_table().get("version") {
            Some(Item::Value(Value::Integer(v))) => {
                if *v.value() != LOCK_FILE_VERSION {
                    bail!("unsupported file format version {version}", version = v.value());
                }
...
impl FileLock {
    ...
    pub fn try_open_ro(path: impl Into<PathBuf>) -> Result<Option<Self>> {
        Self::open(path.into(), OpenOptions::new().read(true), Access::Shared, true)
    }
    ...
    pub fn open_ro(path: impl Into<PathBuf>) -> Result<Self> {
        Ok(Self::open(path.into(), OpenOptions::new().read(true), Access::Shared, false)?.unwrap())
    }
    ...
    fn open(
        ...
        return match res {
            ...
            Err(e) => Err(anyhow!(e)
                .context(format!("failed to lock file `{path}`", path = lock.path.display()))),
        };
...
mod sys {
    ...
    pub(super) fn error_contended(err: &Error) -> bool {
        err.raw_os_error().map_or(false, |x| x == ERROR_LOCK_VIOLATION as i32)
    }

    pub(super) fn error_unsupported(err: &Error) -> bool {
        err.raw_os_error().map_or(false, |x| x == ERROR_INVALID_FUNCTION as i32)
    }
    ...
    fn lock_file(file: &File, flags: u32) -> Result<()> {
        unsafe {
            ...
            let ret = LockFileEx(file.as_raw_handle() as HANDLE, flags, 0, !0, !0, &mut overlapped);
            if ret == 0 {
                Err(Error::last_os_error())
            } else {
                Ok(())
